...
\begin{tabular}{l|l}
	...
	\verb|reps_scalers|         & {{ program._rep_scalers | map
	('round2digits') | join(', ') }} \\
	\verb|intensity_scalers|    & {{ program._intensity_scalers | map
	('round2digits') | join(', ') }} \\
...
\begin{tabular}{llllll}
    ...
    {% for dynamic_ex in day.dynamic_exercises %}
        \hspace{0.5em}{{dynamic_ex.name}} & {{dynamic_ex.start_weight}} {{program.units}} &
         {{dynamic_ex.final_weight}} {{program.units}} & {{dynamic_ex
         .min_reps}} & {{dynamic_ex.max_reps}} &
          {{dynamic_ex.weekly_growth(program.duration)}}\%\\
    {% endfor %}
    {% for static_ex in day.static_exercises %}
       \hspace{0.5em}{{static_ex.name}} & \multicolumn{ 5 }{l}{ {{static_ex.sets_reps(1)}} } \\
      {% endfor %}
{% endfor %}
\end{tabular}
...
{% for day in program.days %}
  ...
  {% for static_ex in day.static_exercises %}
   \hspace{0.75em} {{static_ex.name}} &  \multicolumn{ {{table_width - 1}} }{l}{ {{static_ex.sets_reps(week)}} } \\
  {% endfor %}
  \end{tabular}
